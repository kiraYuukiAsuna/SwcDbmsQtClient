...
void funcParam(const char *par){
// check for function list
	int pos=0;int fun2=-1;
	int fun1=atoi(par);
	//cerr<<"Func:"<<fun1<<"\n";

	const char * find=strchr(par,(int)'f');
	if(find==0){
		fun2=-1;
	}else {
		find++;
	   ...
	   
   {
	...
	for(int i=1;i<argc;i++)
		...
		if(strncmp(argv[i],"-",1)!=0){
			...
			if(strchr(param,'M')!=NULL  && std::filesystem::is_directory(direc)) {
			...
			if(true){
				...
				try {
					for (const auto& entry : std::filesystem::directory_iterator(direc)) {
						...
						int len = strlen(direc) + filename.length();

						list[indexList] = new char[len + 1];
						neuroClass[indexList] = neuronalClass;
						strcpy(list[indexList], direc);
						strcat(list[indexList], filename.c_str());
						indexList++;
					}
				} catch (const std::filesystem::filesystem_error& e) {
					cout << "error!! directory not found: " << e.what();
				}
				} // IF FALSE
			}else {
				//single Neuron
