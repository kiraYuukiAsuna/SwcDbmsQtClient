...
public:
	...
	virtual void printHeader() {
		if (strcmp(a->getName(), "N_Class") != 0) {
			std::cout << a->getName() << "TotSum ";

			std::cout << a->getName() << "Min ";
			std::cout << a->getName() << "Avg ";
			std::cout << a->getName() << "Max ";
	...
	virtual void printDistrDB() {
		if (strcmp(a->getName(), "N_Class") == 0) {
			...
			std::cout << data[tab] / div << ","; //tot

			double minV, maxV, stdV;
			minV = getMin();
			maxV = getMax();
			stdV = getStd();
			double m1, m2;
			...
			m2 = getMean();
			if (fabs(m1 - m2) > 1)
				std::cerr << "error computing mean!\n";
			if (fabs(minV) < 0.000001)
				minV = 0;
			if (fabs(maxV) < 0.000001)
				maxV = 0;
			if (fabs(stdV) < 0.000001)
				stdV = 0;
			if ((minV) > 1000000)
				minV = 999999;
			if ((maxV) > 1000000)
	...
	virtual void printDistr(std::string str) {
		...
		if (strcmp(a->getName(), "Width") == 0
				...
				"Depth") == 0) {
			std::cout << getMax() << "\t" << counter[tab];
		} else {
			std::cout << data[tab] << "\t" << counter[tab];
		}

		std::cout << "\t(" << (nrVoid[tab]-nrVirtual[tab]) << ")";
		std::cout << "\t" << getMin();
		std::cout << "\t" << getMean() << "\t" << getMax() << "\t" << getStd()
				<< " \n";
...
public:
	...
	void computeStep(Segment* t) {
		...
		if (strcmp(a->getName(), b->getName()) == 0 && b2 < LM_VOID) {
			a1 = 1;
		}
		storeValue(a1, b2);
	}
