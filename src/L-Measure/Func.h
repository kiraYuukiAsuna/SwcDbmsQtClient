...
protected:
	...
	Func *limit;
public:
	double virtual computeStep(Segment *t){return LM_VOID;};

	Func(){name="<NoName>";limit=0;pre=this;};
	Func(const char* m){
		name=m;limit=0;pre= this;
	}
	Func(const char*m, Func* l){name=m;limit=l;}
	Func(const char*m, Func* preElaboration, Func* l){name=m;limit=l;pre=preElaboration;}
	...
	const char * getLimit(){if (limit==0) return "";return limit->getName();}
...
public:
	...
	double  computeStep(Segment *t){
		...
		//To avoid the virtual compartments
		if(t->getType() == -1) return -LM_VOID;
		//if(t->getType()== 3) std::cerr<<"found dendrite..";
		return t->getType();}
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t==NULL ||t->getPrev()==NULL) return LM_VOID;
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t->getRadius()!=t->getRadius())return LM_VOID;	
		if(t->getId()<0) return LM_VOID;


		if( t->getPid()==-1 ){

			notSingle=nrSomaSeg(t);
			std::cerr<<"#soma seg:"<<notSingle<<"\n";
		...
		if(notSingle>0){
			...
			if (t->getPrev()!=NULL && t->getType()==1 &&t->getId()>0){
				...
				//std::cerr<<"id:"<<t->getId()<<" soma val:"<<(t->getEnd()->distance(t->getPrev()->getEnd()))*M_PI*t->getPrev()->getDiam()<<"\n";
				std::cerr<<"id:"<<t->getId()<<" soma val:"<<M_PI*t->getPrev()->getDiam()*t->getPrev()->getDiam()<<"\n";
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		//if(length < 0.01)
		//std::cerr<< "Taper_1 length:"<<length<<"\n";
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t->isFather() && t->getType() > 0) {
			max=5;min=0;
			d=t->getDiam();
			if(t->getNext2()==NULL) return LM_VOID;
			d1=t->getNext1()->getDiam();
			d2=t->getNext2()->getDiam();

			if(d==d1 ||d==d2){
				//std::cerr<<t->getId()<<"--barnch invalid for d==d1 ||d==d2 \n";
				return LM_VOID;
			}
			...
			if (minima==0 || minima==max){
				//std::cerr<<t->getId()<<"--barnch invalid for minima 0 or max \n";
				return LM_VOID;
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		//if prev is father return father/segment ratio else LM_VOID
		if(t->getPrev()!=NULL)
	...
	double  computeStep(Segment *t){
		...
		if(t->isFather() ||(t->isTerminate() && t->getType()>0)){
			...
			if(brnchlen == 0){//discard branches that were formed due to triforcation adjustment sri 10/01/2012
				//std::cerr<<"stop here.."<<t->getEnd()->getX()<<","<<t->getEnd()->getY()<<","<<t->getEnd()->getZ()<<" "<<t->getType()<<" "<<brnchlen<<"\n";
				return LM_VOID;
			}
			else if(brnchlen <= 0.1){
				std::cerr<<"branchlen<=0.1 "<<t->getEnd()->getX()<<","<<t->getEnd()->getY()<<","<<t->getEnd()->getZ()<<" "<<t->getType()<<" "<<brnchlen<<"\n";
			}
				return (brnchlen);
		}
...
public:
	...
	double computeStep(Segment *t) {
		...
		if (t->isFather()){
			...
			//std::cout<<"for Id:"<<t->getId()<<" --value:"<<toreturn<<" \n";
			...
			if(toreturn<0)
				std::cerr<<"negative.."<<toreturn<<" "<<t->getEnd()->getX()<<","<<t->getEnd()->getY()<<","<<t->getType()<<" next1 "<<t->getNext1()->getEnd()->getX()<<","<<t->getNext1()->getEnd()->getY()<<","<<t->getNext1()->getEnd()->getZ()<<" next2 "<<t->getNext2()->getEnd()->getX()<<","<<t->getNext2()->getEnd()->getY()<<","<<t->getNext2()->getEnd()->getZ()<<"\n";
			return toreturn;
		}
	...
	double computeMaxAngle( Segment *t_main, Segment * t1, Segment * t) {
		...
		if (tn1_is_virt || tn2_is_virt) {
			...
			if (computedValue > maxValue && computedValue != LM_VOID)
...
//
//			//std::cout<<t->getNonVirtualNext1()->getId()<<","<<t->getNonVirtualNext2()->getId()<<"\n";
//
//			toreturn = (t->getEnd()->angle(t->getNonVirtualNext1()->getEnd(),t->getNonVirtualNext2()->getEnd()));
//			//std::cout<<"for Id:"<<t->getId()<<" --value:"<<toreturn<<" \n";
//			return toreturn;
	...
	double computeStep(Segment *t) {
		...
		if (t->getNextFather2() != NULL)
		{
			if (t->isFather())
			{
				...
				if(ang<0)
						std::cerr<<"possible outlier..reg"<<ang<<"\n";
				return ang;
			}
		}
		//If only Next1 father exists
			...
			if (t->isFather())
			{
				...
				double ang = t->getEnd()->angle(t->getNextFather1()->getEnd(),
						getTip2(t)->getEnd());
				if(ang<0)
						std::cerr<<"possible outlier..bif and tip"<<ang<<"\n";
				return ang;
			}
		...
		//If only Next2 father exists
			...
			if (t->isFather())
			{
				...
				if(ang<0)
						std::cerr<<"possible outlier..tip and next2 father"<<ang<<"\n";
				return ang;
			}
		...
		//if none of the fathers exists
			...
			if (t->isFather())
			{
				...
				double ang =t->getEnd()->angle(getTip1(t)->getEnd(),
						getTip2(t)->getEnd());
				if(ang<0)
						std::cerr<<"possible outlier..two tips"<<ang<<"\n";
				return ang;
			}
		}
		return LM_VOID;
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(rad==0)return LM_VOID;
		double sa=M_PI*rad*rad;

		if(sa==0)return LM_VOID;
		return sa;
...
public:
	...
	double  computeStep(Segment *t){
		...
		if((t->isFather()||(t->isTerminate() && t->getType()>0)) && t->getPrevFather()->getId()>-1){
			...
			if(t->getPrevFather()==NULL) return LM_VOID;
...
public:
	...
	double computeStep(Segment *t){
		...
		if(((t->isFather() && t->getPrev()!=NULL)||(t->isTerminate() && t->getType()>1)) && t->getPrevFather()->getId()>-1){
			...
			//if(t->getType()==1)
			//std::cerr<<t->getEnd()->getZ()<<" "<<t->getType()<<" "<<prevtot->getEnd()->getZ()<<" "<< prevfather->getEnd()->getZ()<<"\n";
			//if(t==prevfather|| prevtot==prevfather || prevtot->getPrev()==prevfather) return LM_VOID;
			//there must be always 3 points available.(i.e) t and prevfather should have atleast one point in between, which is prevtot.
			//if(prevtot==prevfather) return LM_VOID;
			//std::cerr<<"testing fractal points..";
			while(prevtot->getEnd()!=prevfather->getEnd()){
				...
				//std::cerr<<t->getEnd()->getZ()<<" "<<prevtot->getType()<<" "<<eucx[ctr]<<" "<<pathy[ctr]<<" "<<ctr<<"\n";
				ctr++;
	...
	double computeD(double x[], double y[],int size){
		...
		for(int i=size;i>=0;i--){
			...
			if(x[i] < 1 || y[i] < 1){
				//std::cerr<<"value is less than 1..";
				i--;
				size--;

			}
			//std::cerr<<i<<" "<<x[i]<<","<<y[i]<<" "<<log(x[i])<<" "<<log(y[i])<<"\n";
		...
		//std::cout<<"xy:"<<prodofxy<<"Sx:"<<sumofx<<"Sy:"<<sumofy<<"Sx2:"<<sumofx2<<"\n";
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t->getType() < 1) return LM_VOID;
		if(!(t->isFather() && t->getPrevFather()->isFather())) return LM_VOID;

		if(t->getId() == 1){
			return LM_VOID;
		}

		//if( t->getNextFather1()==NULL || t->getNextFather2()==NULL ) return LM_VOID;
		...
		//std::cerr<<prev->getType()<<","<<prev->getEnd()->getX()<<","<<prev->getEnd()->getY()<<","<<prev->getEnd()->getZ()<<"\n";
		...
		if( prev->getNextFather1()!=NULL){
			...
			//std::cerr<<prev_next1->getType()<<","<<prev_next1->getEnd()->getX()<<","<<prev_next1->getEnd()->getY()<<","<<prev_next1->getEnd()->getZ()<<"\n";
		}
		...
		if (prev->getNextFather2() != NULL) {
			...
			//std::cerr<<prev_next2->getType()<<","<<prev_next2->getEnd()->getX()<<","<<prev_next2->getEnd()->getY()<<","<<prev_next2->getEnd()->getZ()<<"\n";
		...
		if (prev_next1 == NULL ||prev_next2 == NULL){
			//std::cerr<<"either next1 or next2 is null"<<"\n";
			return LM_VOID;
		}
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t->getType() < 1) return LM_VOID;
		//still in soma
		if(t->getId()==t->getPrevFather()->getId()) return LM_VOID;
		if(!(t->isFather() && t->getPrevFather()->isFather()) ) return LM_VOID;
		if( t->getPrevFather()==NULL || t->getNext1()==NULL || t->getNext2()==NULL ) return LM_VOID;
		//continue only if the actual segment is father and it has a previous one. 
		//PS: Pay attention previous father CAN BE outside the LIMIT SET!!!!
		Segment *prev=t->getPrevFather();
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(!(t->isFather() && t->getPrevFather()->isFather()) && t->getPid()!=-1) return LM_VOID;
		if( t->getNextFather1()==NULL || t->getNextFather2()==NULL ) return LM_VOID;
		//continue only if the actual segment is father and it has a previous one. 
		//PS: Pay attention previous father CAN BE outside the LIMIT SET!!!!
		Segment *prev=t->getPrevFather();
		if( prev->getNextFather1()==NULL || prev->getNextFather2()==NULL ) return LM_VOID;
		Vector * v1=new Vector();
		//evaluate cross product of previous father
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		if(t->isFather() && t->getType() > 0){
			double p=pow(t->getDiam(),k);
			if(p<0.01)
				//std::cerr<<"Pk diam:"<<t->getDiam()<<" k:"<<k<<" pow:"<<p<<"\n";
...
public:
	...
	double  computeStep(Segment *t){ 
		...
		std::cout<<t->getEnd()->getX()<<"\t"<<t->getEnd()->getY()<<"\t"<<t->getEnd()->getZ()<<"\n";
		return LM_VOID;
	}
}
;
