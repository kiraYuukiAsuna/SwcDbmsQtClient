...
int Neuron::LookForFileType(ifstream* in) {
	const char * AMH_TAG1 = "No.";
	const char * AMH_TAG2 = "points";
	const char * AMH_TAG3 = "Point";
	const char * NL_SOMA_TAG1 = "(CellBody)";
	const char * NL_SOMA_TAG2 = "(Closed)";
	const char * CL_TAG1 = "S";
	const char * SWC_TAG1 = "1";
	const char * SWC_TAG2 = "-1";
	const char * SWC_TAG3 = "2";
	const char * SWC_TAG4 = "1";

	const char * SWC_COMMENT = "#";
	int type = -1;
...
bool Neuron::lookFor(const char * string, ifstream* in) {
	char c[500];
	*in >> c;
	...
	while (in->peek() != -1) {
		...
		*in >> thick;
		...
		if (strcmp("MTO", type) == 0 || strcmp("TTO", type) == 0) {
			...
			} else if (strchr(name, 'x') != NULL) {

				myparid = 1;
				tag = 2;
			} else if (strchr(name, 'a') != NULL) {

				tag = 4;
			} else if (strchr(name, 'b') != NULL) {

				tag = 3;
			} else if (strchr(name, 'o') != NULL) {
...
/* Note: The following functions are now provided by pca.cpp:
   - erhand, vector, matrix, free_vector, free_matrix, tred2, tqli
   They are declared in pca.h which is included at the top of this file. */
