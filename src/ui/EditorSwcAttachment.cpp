...
EditorSwcAttachment::EditorSwcAttachment(const std::string &swcUuid,
										 QWidget *parent)
	...
	connect(ui->OKBtn, &QPushButton::clicked, this, [&]() {
		if (m_IsSwcAttachmentExist) {
			...
			if (WrappedCall::UpdateSwcAttachmentSwcByUuid(
					m_SwcUuid, m_SwcAttachmentUuid, m_SwcAttachmentSwcData,
					response, this)) {
				QMessageBox::information(
					this, "Info", "Update Swc Swc attachment successfully!");
				accept();
			} else {
				QMessageBox::critical(
					this, "Error",
					"Update Swc Swc attachment Failed! " +
						QString::fromStdString(response.metainfo().message()));
			}
		} else {
			proto::CreateSwcAttachmentSwcResponse response;
			if (WrappedCall::CreateSwcAttachmentSwcByUuid(
					m_SwcUuid, m_SwcAttachmentSwcData, response, this)) {
				QMessageBox::information(
					this, "Info", "Create Swc Swc attachment successfully!");
				accept();
			} else {
				QMessageBox::critical(
					this, "Error",
					"Create Swc Swc attachment Failed! " +
						QString::fromStdString(response.metainfo().message()));
			}
		}
	});
	...
	connect(ui->DeleteData, &QPushButton::clicked, this, [&]() {
		...
		m_SwcAttachmentSwcData.erase(m_SwcAttachmentSwcData.begin() +
									 currentRow);
	...
	connect(ui->AddData, &QPushButton::clicked, this, [&]() {
		...
		if (editor.exec() == QDialog::Accepted) {
			...
			swcNodeData.mutable_swcnodeinternaldata()->CopyFrom(
				swcNodeInternalData);
	...
	connect(ui->DeleteBtnFromDbBtn, &QPushButton::clicked, this, [&]() {
		...
		if (WrappedCall::DeleteSwcAttachmentSwcByUuid(
				m_SwcUuid, m_SwcAttachmentUuid, response, this)) {
			QMessageBox::information(this, "Info",
									 "Delete Swc Swc attachment successfully!");
			accept();
		} else {
			QMessageBox::critical(
				this, "Error",
				"Delete Swc Swc attachment Failed! " +
					QString::fromStdString(response.metainfo().message()));
		}
	});
		...
		if (dialog.exec()) {
			...
			if (std::filesystem::exists(path)) {
				...
				try {
					...
					std::for_each(
						value.begin(), value.end(), [&](NeuronUnit &val) {
							proto::SwcNodeDataV1 data;
							auto *internalData =
								data.mutable_swcnodeinternaldata();
	...
	connect(ui->ExportBtn, &QPushButton::clicked, this, [&]() {
		...
		if (std::filesystem::exists(path)) {
			...
			std::for_each(
				m_SwcAttachmentSwcData.begin(), m_SwcAttachmentSwcData.end(),
				[&](proto::SwcNodeDataV1 &val) {
					...
					unit.feature_value =
						val.swcnodeinternaldata().feature_value();
			...
			QMessageBox::critical(this, "Error",
								  "Selected Swc File Save Path Not Exists!");
		}
	});
...
EditorSwcAttachment::~EditorSwcAttachment() { delete ui; }
...
void EditorSwcAttachment::getSwcAttachmentSwc() {
	...
	if (!get_swc_meta_info_response.swcinfo().swcattachmentswcuuid().empty()) {
		...
		QMessageBox::critical(
			this, "Error",
			"No Swc Attachment found! You can create a new swc attchment!");
		return;
	}

	proto::GetSwcAttachmentSwcResponse response;
	m_SwcAttachmentUuid =
		get_swc_meta_info_response.swcinfo().swcattachmentswcuuid();
	if (!WrappedCall::GetSwcAttachmentSwc(m_SwcUuid, m_SwcAttachmentUuid,
										  response, this)) {
		return;
	}
