...
EditorAdminUserManager::EditorAdminUserManager(QWidget* parent)
	...
	auto btnChangePermissionGroup =
		new QPushButton("Change Permission Group", this);

	connect(btnCreate, &QPushButton::clicked, [&]() {
		bool bOk;
		auto newUserName = QInputDialog::getText(
			this, "Create User", "Please input new user name",
			QLineEdit::Normal, "NewUserName", &bOk);
		if (!bOk) {
			return;
		}
		if (newUserName.isEmpty()) {
			QMessageBox::information(this, "Create User",
		...
		auto description = QInputDialog::getText(
			this, "New User", "Please input new user description",
			QLineEdit::Normal, "", &bOk);
		if (!bOk) {
			return;
		}

		auto password = QInputDialog::getText(this, "New User",
		...
		WrappedCall::CreateUser(newUserName.toStdString(),
								password.toStdString(),
								description.toStdString(), rsp2, this);
		...
		for (auto index_map : m_TreeWidget->getIndexMap()) {
			if (index_map.second ==
				...
				auto iter = std::find_if(
					response.userinfo().begin(), response.userinfo().end(),
		...
		for (auto index_map : m_TreeWidget->getIndexMap()) {
			if (index_map.second ==
				...
				if (iter != response.userinfo().end()) {
					int result = QMessageBox::information(
						this, "Warning",
						"Are you sure to change this user's permission group? "
						"User Name: " +
							QString::fromStdString(iter->name()),
						QMessageBox::Yes, QMessageBox::No);
					if (result == QMessageBox::Yes) {
						proto::GetUserByUuidResponse rsp1;
						WrappedCall::GetUserInfoByUuid(iter->base().uuid(),
													   rsp1, this);

						bool bOk;
						auto permissionGroupName = QInputDialog::getText(
							this, "Change Permission Group",
							"Please input permission group name",
						...
						auto pgIter = std::find_if(
							rsp2.permissiongrouplist().begin(),
							rsp2.permissiongrouplist().end(),
							[&](const proto::PermissionGroupMetaInfoV1& value) {
								return value.name() ==
									   permissionGroupName.toStdString();
							});
						...
						rsp1.mutable_userinfo()->set_permissiongroupuuid(
							pgIter->base().uuid());
...
EditorAdminUserManager::~EditorAdminUserManager() { delete ui; }
...
void EditorAdminUserManager::refresh() {
	...
	for (auto& userInfo : response.userinfo()) {
		auto* item =
			m_TreeWidget->addTopItem(userInfo.base().uuid(), userInfo.name(),
									 QIcon(Image::ImageUser), {}, 0);

		auto iter = std::find_if(
			rsp1.permissiongrouplist().begin(),
			rsp1.permissiongrouplist().end(),
			[&](const proto::PermissionGroupMetaInfoV1& value) {
		...
		if (iter != rsp1.permissiongrouplist().end()) {
			...
			item->setText(
				1, QString::fromStdString(userInfo.permissiongroupuuid()));
		}

		if (userInfo.createtime().seconds() < 0) {
			item->setText(2, "N/A");
		} else {
			item->setText(2, QString::fromStdString(timestampToString(
								 userInfo.createtime().seconds())));
		}
