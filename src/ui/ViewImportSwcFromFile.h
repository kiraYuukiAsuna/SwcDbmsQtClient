...
public:
    ...
    UpdateImportUiEvent(int currentRow, std::string swcFilePath, std::string anoFilePath, std::string apoFilePath, std::string swcType, int nodeSize, std::string status)
     : QEvent(TYPE),
    currentRow(currentRow),swcFilePath(swcFilePath),
    anoFilePath(anoFilePath),apoFilePath(apoFilePath),
    swcType(swcType),nodeSize(nodeSize),status(status)
    {
