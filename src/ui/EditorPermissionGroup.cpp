...
EditorPermissionGroup::EditorPermissionGroup(QWidget* parent)
	: QDialog(parent), ui(new Ui::EditorPermissionGroup) {
	ui->setupUi(this);
	...
	connect(btnCreate, &QPushButton::clicked, [&]() {
		bool bOk;
		auto newGroupName = QInputDialog::getText(
			this, "New Permission Group", "Please input new group name",
			QLineEdit::Normal, "NewPermissionGroupName", &bOk);
		if (!bOk) {
			return;
		}
		if (newGroupName.isEmpty()) {
			QMessageBox::information(this, "Permission Group",
		...
		auto description = QInputDialog::getText(
			this, "New Permission Group", "Please input new group description",
			QLineEdit::Normal, "", &bOk);

		m_TreeWidget->addTopItem(newGroupName.toStdString(),
								 newGroupName.toStdString(),
								 QIcon(Image::ImageUserPermission), {});

		proto::CreatePermissionGroupResponse response;
		WrappedCall::CreatePermissionGroup(newGroupName.toStdString(),
										   description.toStdString(), response,
										   this);
		...
		for (auto index_map : m_TreeWidget->getIndexMap()) {
			if (index_map.second ==
				...
				auto iter = std::find_if(
					m_PermissionGroupList.begin(), m_PermissionGroupList.end(),
				...
				if (iter != m_PermissionGroupList.end()) {
					...
					if (result == QMessageBox::Yes) {
						...
						WrappedCall::DeletePermissionGroup(iter->base().uuid(),
														   response, this);
						refresh();
					}
					return;
				...
				QMessageBox::information(this, "Error",
										 "PermissionGroup Selected Not found!");
				return;
			}
		}
	});
	connect(btnCommit, &QPushButton::clicked, [&]() {
		int result =
			QMessageBox::information(this, "Warning",
									 "Are you sure to commit your changes to "
									 "permissions in above groups?",
									 QMessageBox::Yes, QMessageBox::No);
		if (result != QMessageBox::Yes) {
			return;
		}
		...
		for (auto& value : m_PermissionGroupList) {
			...
			if (item) {
				...
				for (int permission = 0; permission < descriptor->field_count();
					...
					auto permissionItem =
						m_TreeWidget->findItem(value.name() + name);
					if (!permissionItem) {
						continue;
					}
			...
			if (!WrappedCall::UpdatePermissionGroup(
					value.base().uuid(), value.name(), value.description(),
					value.ace(), response, this)) {
				return;
			}
		}
	});
...
EditorPermissionGroup::~EditorPermissionGroup() { delete ui; }
...
void EditorPermissionGroup::refresh() {
	...
	for (auto& value : m_PermissionGroupList) {
		auto* item = m_TreeWidget->addTopItem(
			value.name(), value.name(), QIcon(Image::ImageUserPermission), {});
		if (value.name() == "Admin") {
		...
		for (int permission = 0; permission < descriptor->field_count();
			...
			auto name = descriptor->field(permission)->name();
			auto* item =
				m_TreeWidget->addItem(value.name(), value.name() + name, name,
