...
RegisterWindow::RegisterWindow(QWidget* parent) : QDialog(parent), ui(new Ui::RegisterWindow) {
    ...
    ui->label_4->setPixmap(QPixmap::fromImage(QImage(Image::ImageDescription).scaled({32, 32})));
}
...
void RegisterWindow::onRegisterBtnClicked(bool checked) {
    ...
    if (WrappedCall::CreateUser(ui->userNameEditor->text().trimmed().toStdString(), ui->passwordEditor->text().trimmed().toStdString(),
                                ui->descriptionEditor->text().trimmed().toStdString(), response, this)) {
        if (response.metainfo().status()) {
            ...
            QMessageBox::warning(this, "Info",
                                 "Register Failed!" + QString::fromStdString(response.metainfo().message()));
        }
    }
}
