...
EditorSwcSnapshot::EditorSwcSnapshot(const std::string& swcUuid,
									 QWidget* parent)
	...
	connect(ui->Export, &QPushButton::clicked, this, [&]() {
		...
		exportSwcData.push_back(
			{response.swcinfo(),
			 {},
			 true,
			 m_SwcSnapshots.at(currentRow).swcsnapshotcollectionname()});
...
EditorSwcSnapshot::~EditorSwcSnapshot() { delete ui; }
