...
CustomTreeWidgetItem* TreeWidget::addTopItem(const std::string& indexString,
											 const std::string& itemName,
											 QIcon icon, QVariant data,
											 int opColumn) {
...
bool TreeWidget::updateTopItem(const std::string& indexString,
							   const std::string& itemName, QIcon icon,
							   QVariant data, int opColumn) {
...
CustomTreeWidgetItem* TreeWidget::addItem(const std::string& parentIndexString,
...
void TreeWidget::removeItemImpl(CustomTreeWidgetItem* item) {
	...
	for (int i = 0; i < count; i++) {
		CustomTreeWidgetItem* childItem =
			dynamic_cast<CustomTreeWidgetItem*>(item->child(0));  // 删除子节点
...
bool TreeWidget::updateItem(const std::string& indexString,
							const std::string& itemName, int opColumn) {
	auto iter = m_IndexMap.find(indexString);
	if (iter == m_IndexMap.end()) {
		return false;
	}
...
bool TreeWidget::updateItem(const std::string& indexString, QIcon icon,
							int opColumn) {
	auto iter = m_IndexMap.find(indexString);
	if (iter == m_IndexMap.end()) {
		return false;
...
bool TreeWidget::updateItem(const std::string& indexString, QVariant data,
							int opColumn) {
	auto iter = m_IndexMap.find(indexString);
	if (iter == m_IndexMap.end()) {
		return false;
...
CustomTreeWidgetItem* TreeWidget::findItem(const std::string& indexString,
										   int opColumn) {
	auto iter = m_IndexMap.find(indexString);
	if (iter == m_IndexMap.end()) {
		return nullptr;
...
QTreeWidget* TreeWidget::getQTreeWidget() { return m_QTreeWidget; }

std::unordered_map<std::string, CustomTreeWidgetItem*>&
TreeWidget::getIndexMap() {
	return m_IndexMap;
}
