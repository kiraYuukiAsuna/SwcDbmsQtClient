...
class IncrementOperationDelegateModel : public QtNodes::NodeDelegateModel {
	Q_OBJECT
public:
	IncrementOperationDelegateModel() {}

	virtual ~IncrementOperationDelegateModel() {}

	virtual bool captionVisible() const override { return true; }

	virtual QString caption() const override { return _caption; };

	virtual bool portCaptionVisible(QtNodes::PortType,
									QtNodes::PortIndex) const {
		return true;
	}

	virtual QString portCaption(QtNodes::PortType portType,
	...
	virtual QString name() const override {
		return "IncrementOperationDelegateModel";
	};
	...
	virtual QtNodes::ConnectionPolicy portConnectionPolicy(
		QtNodes::PortType portType,
		QtNodes::PortIndex portIndex) const override {
		if (portType == QtNodes::PortType::In) {
			if (portIndex == 0) {
	...
	virtual std::shared_ptr<QtNodes::NodeData> outData(
		QtNodes::PortIndex const port) override {
	...
	QString _caption;
	//
	void setCaption(QString const &caption) override { _caption = caption; }

	virtual void setInternalData(QVariant data) { m_InternalData = data; }

	virtual QVariant getInternalData() { return m_InternalData; }
	...
	//    /**
	//   * The function notifies the Graph Model and makes it remove and
	//   recompute the
	//   * affected connection addresses.
	//   */
	//    void portsAboutToBeDeleted(PortType const portType, PortIndex const
	//    first, PortIndex const last);
	//
	//    /// Call this function when data and port moditications are finished.
	//    void portsDeleted();
	//
	//    /// Call this function before inserting the data associated with
	//    ports.
	//    /**
	//   * The function notifies the Graph Model and makes it recompute the
	//   affected
