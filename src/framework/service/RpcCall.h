...
public:
	...
	void initialize(const std::string& endpoint) {
		...
		channelArgs.SetInt(GRPC_ARG_MAX_RECEIVE_MESSAGE_LENGTH,
						   256 * 1024 * 1024);	// 256mb
		...
		m_GrpcContextThread = std::thread([this]() { m_GrpcContext->run(); });
	}
	...
	auto& Endpoint() { return m_Endpoint; }

	auto& Channel() { return m_Channel; }

	auto& Stub() { return m_Stub; }

	auto& GrpcContext() { return m_GrpcContext; }
...
private:
	...
	std::shared_ptr<agrpc::GrpcContext> m_GrpcContext;
