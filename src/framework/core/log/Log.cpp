...
namespace Seele {
	...
	void Log::Init(const std::string& logsDirectory,
				   ...
				   const std::string& editorConsoleLogsFileName,
				   bool hasConsole, bool truncate) {
		...
		std::string editorConsoleLogsFile =
			logsDirectory + "/" + editorConsoleLogsFileName;
		...
		std::vector<spdlog::sink_ptr> coreSinks = {
			std::make_shared<spdlog::sinks::basic_file_sink_mt>(coreLogsFile,
																truncate),
		};
		if (m_HasConsole) {
			coreSinks.push_back(
		...
		std::vector<spdlog::sink_ptr> appSinks = {
			std::make_shared<spdlog::sinks::basic_file_sink_mt>(appLogsFile,
																truncate),
		};
		if (m_HasConsole) {
			appSinks.push_back(
		...
		std::vector<spdlog::sink_ptr> editorConsoleSinks = {
			std::make_shared<spdlog::sinks::basic_file_sink_mt>(
				editorConsoleLogsFile, truncate),
		};
		if (m_HasConsole) {
			editorConsoleSinks.push_back(
		...
		s_CoreLogger = std::make_shared<spdlog::logger>(
			"SeeleCoreLogger", coreSinks.begin(), coreSinks.end());
		s_CoreLogger->set_level(spdlog::level::trace);
		s_CoreLogger->flush_on(spdlog::level::err);

		s_AppLogger = std::make_shared<spdlog::logger>(
			"SeeleAppLogger", appSinks.begin(), appSinks.end());
		s_AppLogger->set_level(spdlog::level::trace);
		s_AppLogger->flush_on(spdlog::level::err);

		s_EditorConsoleLogger = std::make_shared<spdlog::logger>(
			"SeeleEditorConsoleLogger", editorConsoleSinks.begin(),
