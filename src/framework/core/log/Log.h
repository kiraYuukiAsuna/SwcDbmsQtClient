...
namespace Seele {
	class Log {
	public:
		enum class Type : uint8_t { Core = 0, App };
		...
		static void Init(
			...
			const std::string& editorConsoleLogsFileName = "EditorConsole.log",
			bool hasConsole = true, bool truncate = true);
...
// Core logging
template <typename... Args>
void SeeleCoreTraceTag(const char* tag,
					   const std::format_string<Args...>& format,
					   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::Core,
							   ::Seele::Log::Level::Trace, tag, format,
							   std::forward<Args>(args)...);
}

template <typename... Args>
void SeeleCoreInfoTag(const char* tag,
					  const std::format_string<Args...>& format,
					  Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::Core,
							   ::Seele::Log::Level::Info, tag, format,
							   std::forward<Args>(args)...);
}

template <typename... Args>
void SeeleCoreWarnTag(const char* tag,
					  const std::format_string<Args...>& format,
					  Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::Core,
							   ::Seele::Log::Level::Warn, tag, format,
							   std::forward<Args>(args)...);
}

template <typename... Args>
void SeeleCoreErrorTag(const char* tag,
					   const std::format_string<Args...>& format,
					   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::Core,
							   ::Seele::Log::Level::Error, tag, format,
							   std::forward<Args>(args)...);
}

template <typename... Args>
void SeeleCoreFatalTag(const char* tag,
					   const std::format_string<Args...>& format,
					   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::Core,
							   ::Seele::Log::Level::Fatal, tag, format,
							   std::forward<Args>(args)...);
}
// App logging
template <typename... Args>
void SeeleTraceTag(const char* tag, const std::format_string<Args...>& format,
				   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::App,
							   ::Seele::Log::Level::Trace, tag, format,
							   std::forward<Args>(args)...);
}

template <typename... Args>
void SeeleInfoTag(const char* tag, const std::format_string<Args...>& format,
				  Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::App,
							   ::Seele::Log::Level::Info, tag, format,
							   std::forward<Args>(args)...);
...
template <typename... Args>
void SeeleWarnTag(const char* tag, const std::format_string<Args...>& format,
				  Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::App,
							   ::Seele::Log::Level::Warn, tag, format,
							   std::forward<Args>(args)...);
...
template <typename... Args>
void SeeleErrorTag(const char* tag, const std::format_string<Args...>& format,
				   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::App,
							   ::Seele::Log::Level::Error, tag, format,
							   std::forward<Args>(args)...);
...
template <typename... Args>
void SeeleFatalTag(const char* tag, const std::format_string<Args...>& format,
				   Args&&... args) {
	::Seele::Log::PrintMessage(::Seele::Log::Type::App,
							   ::Seele::Log::Level::Fatal, tag, format,
							   std::forward<Args>(args)...);
...
namespace Seele {
	...
	void Log::PrintMessage(Log::Type type, Log::Level level,
		...
		if (detail.Enabled && detail.LevelFilter <= level) {
			auto logger =
				(type == Type::Core) ? GetCoreLogger() : GetAppLogger();
			auto userLogString =
				std::format(format, std::forward<Args>(args)...);
			auto finaLogString =
				tag.empty() ? std::format("{}{}", tag, userLogString)
