...
namespace acss {
	...
	private:
		...
		QByteArray
			m_SvgContent;  ///< memory buffer with SVG data load from file
	...
	public:
		...
		explicit CSVGIconEngine(const QByteArray& SvgContent,
								QtAdvancedStylesheet* Styleeheet)
			: m_SvgTemplate(SvgContent), m_AdvancedStyleheet(Styleeheet) {
			update();
		...
		virtual QPixmap pixmap(const QSize& size, QIcon::Mode mode,
			...
			// This function is necessary to create an EMPTY pixmap. It's called
			// always before paint()
	...
	struct QtAdvancedStylesheetPrivate {
		...
		QMap<QString, QString>
			ThemeVariables;	 // theme variables contains StyleVariables and
							 // ThemeColors
		QString Stylesheet;
		 ...
		 */
		bool storeStylesheet(const QString& Stylesheet,
							 const QString& Filename);

		/**
		 * Parse a list of theme variables
		 */
		bool parseVariablesFromXml(QXmlStreamReader& s,
								   const QString& VariableTagName,
		 ...
		 */
		void setError(QtAdvancedStylesheet::eError Error,
		 ...
		 */
		void parsePaletteColorGroup(QJsonObject& jPalette,
									QPalette::ColorGroup ColorGroup);

		/**
		 * Use this function to access the icon color replace list, to ensure,
		 * that is is properly initialized
		 */
		 ...
		 */
		tColorReplaceList parseColorReplaceList(
	...
	//============================================================================
	void QtAdvancedStylesheetPrivate::setError(
		QtAdvancedStylesheet::eError Error, const QString& ErrorString) {
		this->Error = Error;
		this->ErrorString = ErrorString;
		if (Error != QtAdvancedStylesheet::NoError) {
			qDebug() << "CAdvancedStylesheet Error: " << Error << " "
					 << ErrorString;
		}
	}

	//============================================================================
	QString QtAdvancedStylesheetPrivate::rgbaColor(const QString& RgbColor,
												   float Opacity) {
	...
	//============================================================================
	void QtAdvancedStylesheetPrivate::replaceStylesheetVariables(
		QString& Content) {
		...
		while ((index = Content.indexOf(re, index, &match)) != -1) {
			...
			// Use only the value inside of the brackets {{ }} without the
			// brackets
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
			auto TemplateVariable =
				MatchString.midRef(2, MatchString.size() - 4);
#else
			auto TemplateVariable =
				QStringView(MatchString).sliced(2, MatchString.size() - 4);
#endif
			...
			if (HasOpacity) {
				...
				auto OpacityStr = Values[1].mid(
					OpacityStrSize, Values[1].size() - OpacityStrSize - 1);
				bool Ok;
				auto Opacity = OpacityStr.toFloat(&Ok);
				ValueString = rgbaColor(ValueString, Opacity);
			} else {
				ValueString =
					_this->themeVariableValue(TemplateVariable.toString());
			}
	...
	bool QtAdvancedStylesheetPrivate::generateStylesheet() {
		auto CssTemplateFileName =
			JsonStyleParam.value("css_template").toString();
		if (CssTemplateFileName.isEmpty()) {
			return false;
		}

		QString TemplateFilePath =
			_this->currentStylePath() + "/" + CssTemplateFileName;
		if (!QFile::exists(TemplateFilePath)) {
			setError(QtAdvancedStylesheet::CssTemplateError,
					 "Stylesheet folder "
		...
		QFile TemplateFile(_this->currentStylePath() + "/" +
						   CssTemplateFileName);
		TemplateFile.open(QIODevice::ReadOnly);
		QString Content(TemplateFile.readAll());
		replaceStylesheetVariables(Content);
		Stylesheet = Content;
		exportInternalStylesheet(QFileInfo(TemplateFilePath).baseName() +
								 ".css");
		return true;
	}

	//============================================================================
	bool QtAdvancedStylesheetPrivate::exportInternalStylesheet(
		const QString& Filename) {
		return storeStylesheet(this->Stylesheet, Filename);
	}

	//============================================================================
	bool QtAdvancedStylesheetPrivate::storeStylesheet(const QString& Stylesheet,
													  const QString& Filename) {
		...
		if (!OutputFile.open(QIODevice::WriteOnly)) {
			setError(QtAdvancedStylesheet::CssExportError,
					 "Exporting stylesheet " + Filename +
						 " caused error: " + OutputFile.errorString());
			return false;
		}
	...
	void QtAdvancedStylesheetPrivate::addFonts(QDir* Dir) {
		...
		// widgets. The call to QFontDatabase::addApplicationFont() will crash,
	...
	bool QtAdvancedStylesheetPrivate::parseVariablesFromXml(
		QXmlStreamReader& s, const QString& TagName,
		QMap<QString, QString>& Variables) {
		while (s.readNextStartElement()) {
			if (s.name() != TagName) {
				setError(QtAdvancedStylesheet::ThemeXmlError,
						 "Malformed theme "
						 "file - expected tag <" +
							 TagName + "> instead of " + s.name().toString());
				return false;
			}
	...
	bool QtAdvancedStylesheetPrivate::parseThemeFile(const QString& Theme) {
		QString ThemeFileName =
			_this->path(QtAdvancedStylesheet::ThemesLocation) + "/" + Theme;
		...
		if (s.name() != QLatin1String("resources")) {
			setError(QtAdvancedStylesheet::ThemeXmlError,
					 "Malformed theme file - "
					 "expected tag <resources> instead of " +
						 s.name().toString());
			return false;
		}
	...
	bool QtAdvancedStylesheetPrivate::parseStyleJsonFile() {
		...
		if (JsonFiles.count() < 1) {
			setError(QtAdvancedStylesheet::StyleJsonError,
					 "Stylesheet folder does "
	...
	//============================================================================
	void QtAdvancedStylesheetPrivate::parsePaletteColorGroup(
		QJsonObject& jPalette, QPalette::ColorGroup ColorGroup) {
		auto jColorGroup =
			jPalette.value(colorGroupString(ColorGroup)).toObject();
		if (jColorGroup.isEmpty()) {
			return;
		}

		for (auto itc = jColorGroup.constBegin(); itc != jColorGroup.constEnd();
			 ++itc) {
			...
			this->PaletteColors.append(
				{ColorGroup, ColorRole, itc.value().toString()});
	...
	//============================================================================
	void QtAdvancedStylesheetPrivate::replaceColor(
		QByteArray& Content, const QString& TemplateColor,
		const QString& ThemeColor) const {
		Content.replace(TemplateColor.toLatin1(), ThemeColor.toLatin1());
	}

	//============================================================================
	tColorReplaceList QtAdvancedStylesheetPrivate::parseColorReplaceList(
		const QJsonObject& JsonObject) const {
		// Fill the color replace list with the values read from style json file
		tColorReplaceList ColorReplaceList;
		for (auto it = JsonObject.constBegin(); it != JsonObject.constEnd();
			 ++it) {
			auto TemplateColor = it.key();
			auto ThemeColor = it.value().toString();
			// If the color starts with an hashtag, then we have a real color
			// value If it does not start with # then it is a theme variable
	...
	//============================================================================
	bool QtAdvancedStylesheetPrivate::generateResourcesFor(
		const QString& SubDir, const QJsonObject& JsonObject,
		const QFileInfoList& Entries) {
		const QString OutputDir =
			_this->currentStyleOutputPath() + "/" + SubDir;
		if (!QDir().mkpath(OutputDir)) {
			setError(QtAdvancedStylesheet::ResourceGeneratorError,
					 "Error "
					 "creating resource output folder: " +
						 OutputDir);
			return false;
		}
	...
	//============================================================================
	const tColorReplaceList& QtAdvancedStylesheetPrivate::iconColorReplaceList()
		const {
	...
	//============================================================================
	void QtAdvancedStylesheet::replaceSvgColors(
		QByteArray& SvgContent, const tColorReplaceList& ColorReplaceList) {
		const tColorReplaceList& ReplaceList = ColorReplaceList.isEmpty()
												   ? d->iconColorReplaceList()
												   : ColorReplaceList;
	...
	QtAdvancedStylesheet::QtAdvancedStylesheet(QObject* parent)
		: QObject(parent), d(new QtAdvancedStylesheetPrivate(this)) {}

	//============================================================================
	QtAdvancedStylesheet::~QtAdvancedStylesheet() { delete d; }
	...
	//============================================================================
	QString QtAdvancedStylesheet::stylesDirPath() const { return d->StylesDir; }
	...
	//============================================================================
	QString QtAdvancedStylesheet::outputDirPath() const { return d->OutputDir; }
	...
	//============================================================================
	QString QtAdvancedStylesheet::themeVariableValue(
	...
	//============================================================================
	void QtAdvancedStylesheet::setThemeVariableValue(const QString& VariableId,
													 const QString& Value) {
	...
	bool QtAdvancedStylesheet::updateStylesheet() {
		...
		if (!d->generateStylesheet() &&
			(error() != QtAdvancedStylesheet::NoError)) {
			return false;
		}
	...
	//============================================================================
	QString QtAdvancedStylesheet::styleSheet() const { return d->Stylesheet; }
	...
	//============================================================================
	QString QtAdvancedStylesheet::processStylesheetTemplate(
		const QString& Template, const QString& OutputFile) {
	...
	//============================================================================
	const QMap<QString, QString>& QtAdvancedStylesheet::themeColorVariables()
		const {
	...
	//============================================================================
	QString QtAdvancedStylesheet::errorString() const { return d->ErrorString; }
	...
	bool QtAdvancedStylesheet::generateResources() {
		...
		if (jresources.isEmpty()) {
			d->setError(QtAdvancedStylesheet::StyleJsonError,
						"Key resources "
		...
		bool Result = true;
		for (auto itc = jresources.constBegin(); itc != jresources.constEnd();
			 ++itc) {
	...
	QPalette QtAdvancedStylesheet::generateThemePalette() const {
		...
		for (const auto& Entry : d->PaletteColors) {
			...
			if (Color.isValid()) {
				Palette.setColor(Entry.Group, Entry.Role,
								 themeColor(Entry.ColorVariable));
			}
		}
