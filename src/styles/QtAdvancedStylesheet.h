...
namespace acss {
	...
	public:
		...
		/**
		 * Sets the output directory path where the generated theme will be
		 * stored
		 */
		 ...
		 * You may use this function to add application specific colors that are
		 * not defined in the current style. If you changed a theme variable or
		 * a number of theme variables then you should call updateStylesheet()
		 * to request a reprocessing of the style template and to update the
		 * stylesheet.
		 */
		void setThemeVariableValue(const QString& VariableId,
								   const QString& Value);

		/**
		 * Returns the color for the given VariableId.
		 * If VariableId is not a color variable, then this function returns an
		 * invalid QColor.
		 */
		 ...
		 * several parts or if you use plugins that provide additional
		 * stylesheets to get the processed stylesheed if the theme changed.
		 ...
		 * will be stored into the currentStyleOutputPath() folder with the
		 * given OutputFile name.
		 */
		QString processStylesheetTemplate(
			const QString& Template, const QString& OutputFile = QString());

		/**
		 * Returns the icon for the current style or an empty icon if the style
		...
		/**
		 * This function creates a palette with the theme colors of the
		 ...
		 * If an optional ColorReplaceList is provided, then the given list is
		 * used to replace the colors in the given SvgContent file. If no color
		 * replace list is given, the internal color replace list parsed from
		 * style json file is used
		 */
		void replaceSvgColors(
	...
	public slots:
		 ...
		 * Call this function, if you have changed the theme or some theme
		 * variables via setThemeVariable() to request an update of the
		 * stylesheet. The function updates the application palette with the
		 * theme colors (updateApplicationPaletteColors()), generates the theme
		 * SVG rseouces (generateResources()) and then generates the stylesheet
		 * if it has a template file. If you have split your stylesheet into
		 * several pieces or if your would like to separate the update of the
		 * resources and the generation of the stylehseets, then you should not
		 * use this function. Instead, call processStyleTemplate() to generate
		 * the SVG resources and update the palette an then use
		 * processStylesheetTemplate() to generate the style sheets.
		 */
		 ...
		 * The functio will not update or create any stylesheet. After calling
		 * this function, you can use the processStylesheetTemplate() function
		 * to manually generate the style sheets from stylesheet template files.
		 */
		bool processStyleTemplate();

		/**
		 * Generate the required icons for this theme.
		 * You can call this function, if you changed some theme settings or
		 * theme variables and if you would like to recreate the svg resources
		 * of this style.If you call updateStylesheet(), then this function will
		 ...
		 * This function is called automatically if updateStylesheet() is
		 * called. The function creates a palette with theme colors via
		 * generateThemePalette() and then assigns the platte to the application
		 * object.
		 */
	...
	signals:
		 ...
		 * The stylesheed changes if the style changes, the theme changes or if
		 * a style variable changed an the user requested a styleheet update
		 */
