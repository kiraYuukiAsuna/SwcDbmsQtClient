...
void resample_path(Segment * seg, double step)
{
	...
	while (iter_old < (V3DLONG)seg->size() && start && start->p)
	{
		path_length += DISTP(start,start->p);
		if (path_length<=(double)seg_r.size()*step)
		{
			start = start->p;
			iter_old++;
		}
		else//a new point should be created
		{
			...
			pt->r = start->r*(1-rate) + start->p->r*rate;
			pt->p = start->p;
                        pt->type = start->type;
...
NeuronTree resample(NeuronTree input, double step)
{
	...
	for (V3DLONG i=0;i<(V3DLONG)seg_list.size();i++)
	{
	...
	for (V3DLONG j=0;j<(V3DLONG)seg_list.size();j++)
